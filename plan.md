...
To support multiple firms (5 to 100+ users per firm) while maintaining strict data isolation, the architecture will migrate to a centralized GCP backend.
...
- **Row-Level Security:** GCP Firestore (or Cloud SQL) security rules will mathematically prevent User A at Firm X from querying data belonging to Firm Y.
- **Authentication:** Migrate from local mock auth to Google Identity Platform (Firebase Auth) to handle enterprise SSO, MFA, and secure session tokens.
...
- **Seat Caps:** When a firm's IT Admin attempts to provision a new user, the GCP backend will check their current active users against their `subscription_tier` limit (e.g., 20 users). If exceeded, the provisioning API call is rejected.
...
### 4.1 The GCP Middleware API
- **Cloud Functions / Cloud Run:** The desktop app will *never* communicate directly with Google Gemini or OpenAI. It will send requests to a custom Rayn API hosted on GCP.
- **Key Protection:** The Rayn GCP backend holds the actual AI API keys securely in Google Secret Manager.
...
- **Execution & Logging:** If within limits, the backend queries the AI, returns the drafted text to the desktop client, and increments the firm's token usage in the billing database.

## 5. External Integrations (Phase 4) - PENDING
- **OAuth 2.0 Flow:** Establishing a Google Cloud OAuth Consent screen to allow lawyers to securely connect their firm's Google Workspace to export drafts to Docs and send emails.
- **Webhooks:** Finalizing the server-side endpoints to handle incoming/outgoing WhatsApp Business API messages for the Client Portal.

## 6. Enterprise Multi-Agent AI System (Phase 5) - PENDING
The simple AI API gateway will be upgraded into a proactive, enterprise-grade Agentic RAG system built on **CrewAI** and **LangChain**, hosted on GCP Cloud Run.
...
- **Private Firm Knowledge Base:** A multi-tenant Vector Database (`firm_id` isolated) where user-uploaded case files, evidence, and historical firm documents are stored securely. 
...
- **The "Watchdog" Agent:** A scheduled CrewAI task equipped with Web Search tools (e.g., Serper API) that continuously monitors official Indian government gazettes and Supreme Court portals.
...
- **Feedback Loops:** When a lawyer manually edits an AI-generated draft in the Desktop App, the diff (changes made) is sent back to the GCP backend.
- **Firm Preferences DB:** The backend extracts the stylistic or legal corrections and updates a "Preferences Vector Profile" specific to that `firm_id`. Future drafting agents will load this profile into their context window, ensuring the AI learns the specific partner's writing style over time.
...
2. **Document Storage:** Defining the bucket architecture (Google Cloud Storage) for handling large legal PDFs and evidence files securely per firm.

---
*Note: This plan serves as the architectural blueprint. Each phase will require specific implementation sprints, starting with setting up the GCP infrastructure and migrating the local database logic to cloud APIs.*